    ...
    <div class="bg-card rounded-2xl border border-line shadow-brandMd p-5 space-y-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold text-textDark">Disbursement Management</h1>
            </div>
            <?php if (hasRole(['ROLE_ACCOUNTANT', 'ROLE_ADMIN'])): ?>
                <a href="<?= BASE_URL ?>/views/shared/disbursement/create.php"
                   class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-orange text-navy900 text-sm font-bold hover:bg-orangeDark transition-colors no-underline">
                    <i class='bx bx-plus'></i>New Disbursement Request
                </a>
            <?php endif; ?>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5">
                <div class="flex items-start justify-between mb-4">
                    <span class="text-[.7rem] font-bold text-textMid uppercase tracking-wide">Total Requested</span>
                ...
                <div class="text-2xl font-extrabold text-textDark mb-1" id="statTotalRequested">₱ 0.00</div>
                <div class="text-xs text-textSoft" id="statRequestCount">0 requests</div>
            </div>
            <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5">
                <div class="flex items-start justify-between mb-4">
                    <span class="text-[.7rem] font-bold text-textMid uppercase tracking-wide">Awaiting Approval</span>
            ...
            <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5">
                <div class="flex items-start justify-between mb-4">
                    <span class="text-[.7rem] font-bold text-textMid uppercase tracking-wide">Released</span>
                ...
                <div class="text-2xl font-extrabold text-textDark mb-1" id="statReleasedAmount">₱ 0.00</div>
                <div class="text-xs text-textSoft" id="statReleasedCount">0 released</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-card rounded-2xl border border-line shadow-brandSm overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4 border-b border-line">
            <div class="flex items-center gap-2 text-sm font-semibold" id="statusTabs">
                ...
                <button type="button" data-status="Approved" class="status-tab px-4 py-2 rounded-full bg-[#eef1f5] text-textMid hover:bg-[#e4e8ee]">Approved</button>
                <button type="button" data-status="Released" class="status-tab px-4 py-2 rounded-full bg-[#eef1f5] text-textMid hover:bg-[#e4e8ee]">Released</button>
            ...
            <div class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" placeholder="Search supplier or invoice..."
            ...
            <table class="w-full text-sm border-separate border-spacing-0">
                <thead>
                    <tr class="text-left text-[11px] uppercase tracking-wide text-textSoft border-b border-line">
                        ...
                        <th class="px-2 py-3 font-semibold">AP Invoice</th>
...
const canApprove = <?= hasRole(['ROLE_FINANCE', 'ROLE_ADMIN']) ? 'true' : 'false' ?>;
...
function updateStats(disbursements) {
    const totalRequested = disbursements.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0);
    ...
    const releasedAmount = released.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0);

    document.getElementById('statTotalRequested').textContent = currency(totalRequested);
    document.getElementById('statRequestCount').textContent = disbursements.length + ' request' + (disbursements.length === 1 ? '' : 's');
    ...
    document.getElementById('statReleasedAmount').textContent = currency(releasedAmount);
    document.getElementById('statReleasedCount').textContent = released.length + ' released';
}

function renderRows(disbursements) {
    const tbody = document.getElementById('disbTableBody');

    if (disbursements.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="px-5 py-8 text-center text-textSoft">No disbursements found.</td></tr>';
        return;
    }
    ...
    disbursements.forEach(d => {
        ...
        if (d.status === 'Pending' && canApprove) {
            actions = `<a href="${BASE_URL}/views/shared/disbursement/approve.php?disbursement_id=${d.disbursement_id}"
                          ...
                          <i class='bx bx-check'></i>Approve</a>`;
        }

        const badgeClass = STATUS_BADGE[d.status] || 'bg-[#eef1f6] text-textMid';

        tr.innerHTML = `
            <td class="px-5 py-3 align-top text-textMid whitespace-nowrap">${d.disbursement_date}</td>
            <td class="px-2 py-3 align-top text-textDark font-semibold">${d.supplier_name}</td>
            <td class="px-2 py-3 align-top text-textMid whitespace-nowrap">${d.invoice_no}</td>
            <td class="px-2 py-3 align-top text-textMid">${d.bank_account_name}</td>
            <td class="px-2 py-3 align-top text-right font-semibold text-textDark whitespace-nowrap">${currency(d.amount)}</td>
            <td class="px-2 py-3 align-top text-textMid whitespace-nowrap">${d.payment_method}</td>
            <td class="px-2 py-3 align-top"><span class="px-2.5 py-1 rounded-full text-[11px] font-semibold ${badgeClass}">${d.status}</span></td>
...
function applySearch() {
    ...
    if (!term) {
        renderRows(currentDisbursements);
        return;
    }
    const filtered = currentDisbursements.filter(d =>
        d.supplier_name.toLowerCase().includes(term) ||
        d.invoice_no.toLowerCase().includes(term)
    );
    renderRows(filtered);
}

async function loadDisbursements(status = '') {
    const tbody = document.getElementById('disbTableBody');
    ...
    try {
        ...
        currentDisbursements = data.disbursements;
        updateStats(currentDisbursements);
        renderRows(currentDisbursements);

    } catch (err) {
        tbody.innerHTML = '<tr><td colspan="8" class="px-5 py-8 text-center text-rose-600">Something went wrong loading disbursements.</td></tr>';
    }
}
...
statusTabs.forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        loadDisbursements(status);
    });
});

statusFilter.addEventListener('change', function () {
    setActiveTab(this.value);
    loadDisbursements(this.value);
});
