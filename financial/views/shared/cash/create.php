...
<div class="p-6 space-y-6">
    ...
    <?php if (empty($bankAccounts)): ?>
        <div class="bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            No active bank accounts found. Add one first.
        </div>
    <?php endif; ?>
    ...
    <form id="cashForm" class="space-y-6">
        ...
        <div class="bg-card rounded-2xl border border-line shadow-brandMd p-5 space-y-6">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold text-textDark">New Cash Transaction</h1>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Transaction Type</label>
                    <select name="transaction_type" id="transaction_type" required
                        ...
                        <option value="Deposit">Deposit (money coming in)</option>
                        <option value="Withdrawal">Withdrawal (money going out)</option>
                ...
                <div>
                    <label class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Transaction Date</label>
        ...
        <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Bank/Cash GL Account</label>
                ...
                <div>
                    <label id="contraLabel" class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Contra Account (the other side of the entry)</label>
        ...
        <div class="flex justify-end">
            <button type="submit"
                ...
                <i class='bx bx-check-double'></i>Save & Post Transaction
            </button>
        </div>
    </form>
</div>
...
document.getElementById('transaction_type').addEventListener('change', function () {
    const label = document.getElementById('contraLabel');
    const incoming = ['Deposit', 'Transfer In'].includes(this.value);
    label.textContent = incoming
        ? 'Contra Account (source of the incoming funds — e.g. Revenue, Owner Equity, other bank account)'
        : 'Contra Account (destination of the outgoing funds — e.g. Expense, Owner Drawing, other bank account)';
});

document.getElementById('cashForm').addEventListener('submit', async function (e) {
    e.preventDefault();

    if (!confirm('This will post the GL entry and update the bank balance immediately. Continue?')) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            window.location.href = BASE_URL + '/views/shared/cash/cash-list.php';
        } else {
