...
<div class="p-6 space-y-6">
    ...
    <?php if (empty($customers)): ?>
        <div class="bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            No active customers found. Add a customer first before creating an AR invoice.
        </div>
    <?php endif; ?>

    <form id="arForm" class="space-y-6">
        ...
        <div class="bg-card rounded-2xl border border-line shadow-brandMd p-5 space-y-6">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold text-textDark">New AR Invoice</h1>
            </div>
        ...
        <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Accounts Receivable Account (Debit)</label>
                ...
                <div>
                    <label class="block text-[.7rem] font-bold text-textMid uppercase tracking-wide mb-1.5">Revenue Account (Credit)</label>
...
document.getElementById('arForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (data.success) {
            alert(data.message + ' (AR #' + data.ar_id + ', Journal #' + data.journal_id + ')');
            window.location.href = BASE_URL + '/views/shared/ar/ar-list.php';
        } else {
