...
<script>
    ...
    function statCard(label, value, sub, icon, pct) {
        ...
        <div class="bg-card rounded-2xl border border-line shadow-brandSm p-5">
            <div class="flex items-start justify-between mb-4">
                <span class="text-[.7rem] font-bold text-textMid uppercase tracking-wide">${label}</span>
                <div class="w-9 h-9 rounded-lg bg-navy900 flex items-center justify-center text-orange text-base shrink-0"><i class='bx ${icon}'></i></div>
            </div>
            <div class="text-2xl font-extrabold text-textDark mb-1">${value}</div>
            <div class="text-xs text-textSoft mb-3">${sub}</div>
            <div class="h-1.5 w-full bg-[#eef1f6] rounded-full overflow-hidden"><div class="h-full bg-orange rounded-full" style="width:${pct}%"></div></div>
        </div>`;
    }

    function ledgerRow(je, date, accounts, tag, desc, debit, credit, status, opts) {
        const statusStyles = {
            Void:   'bg-rose-100 text-rose-600',
            Posted: 'bg-emerald-100 text-emerald-600',
            Draft:  'bg-amber-100 text-amber-600',
        };
        const creditColor = { Void: 'text-rose-600', Posted: 'text-emerald-600', Draft: 'text-amber-600' }[status];
        const editBtn = status === 'Void' ? '' : `<button class="w-8 h-8 rounded-lg hover:bg-[#eef1f6] inline-flex items-center justify-center text-orange"><i class='bx bx-edit'></i></button>`;
        return `
        <tr class="border-b border-line hover:bg-[#f8fafc] ${opts || ''}">
            <td class="px-5 py-3 align-top"><input type="checkbox" class="rounded border-line"></td>
            <td class="px-2 py-3 align-top font-semibold text-textDark whitespace-nowrap">${je}</td>
            <td class="px-2 py-3 align-top text-textMid whitespace-nowrap">${date}</td>
            <td class="px-2 py-3 align-top">
                <div class="text-textDark font-medium">${accounts}</div>
                <span class="inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full bg-[#eef1f6] text-textMid">${tag}</span>
            </td>
            <td class="px-2 py-3 align-top text-textMid max-w-[220px] truncate">${desc}</td>
            <td class="px-2 py-3 align-top text-right font-semibold text-textDark whitespace-nowrap">${debit}</td>
            <td class="px-2 py-3 align-top text-right font-semibold whitespace-nowrap ${creditColor}">${credit}</td>
            <td class="px-2 py-3 align-top"><span class="px-2.5 py-1 rounded-full text-[11px] font-semibold ${statusStyles[status]}">${status}</span></td>
            <td class="px-5 py-3 align-top text-right whitespace-nowrap">
                <button class="w-8 h-8 rounded-lg hover:bg-[#eef1f6] inline-flex items-center justify-center text-orange"><i class='bx bx-show'></i></button>
                ${editBtn}
            </td>
        </tr>`;
    }
    ...
    function renderGeneralLedger() {
        const rows = [
            ['JE-2025-1307', '2025-03-01', 'Petty Cash Fund, Cash in Bank', 'General', 'Petty cash replenishment', '₱ 5,000.00', '₱ 5,000.00', 'Void'],
            ['JE-2025-1306', '2025-02-12', 'Utilities Expense, Accounts Payable', 'General', 'Utility expense — electricity (ME...', '₱ 12,300.00', '₱ 12,300.00', 'Posted'],
            ['JE-2025-1305', '2025-02-05', 'Depreciation Expense, Accumulated Depreciation', 'Adjusting', 'Depreciation — office equipment...', '₱ 8,500.00', '₱ 8,500.00', 'Draft'],
        ];
        const highlighted = rows.map((r, i) => ledgerRow(...r,
            i === 0 ? 'border-2 border-b-0 border-blue-500 rounded-t-xl' :
            i === rows.length - 1 ? 'border-2 border-t-0 border-blue-500 rounded-b-xl' :
            'border-x-2 border-blue-500')).join('');
        const repeated = [...rows, rows[0]].map(r => ledgerRow(...r)).join('');
        ...
        <div class="p-6 space-y-6">
            <div class="bg-card rounded-2xl border border-line shadow-brandMd p-5 space-y-6">
                <div class="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#eef1f5] text-textMid text-sm font-semibold hover:bg-[#e4e8ee] transition-colors"><i class='bx bx-download text-orange'></i>Import</button>
                        <button class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-orange text-navy900 text-sm font-bold hover:bg-orangeDark transition-colors"><i class='bx bx-plus'></i>New Journal Entry</button>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ${statCard('Total Debit', '₱ 750,800.00', '6 Active', 'bx-trending-down', 100)}
                    ${statCard('Total Credit', '₱ 750,800.00', '1 Voided', 'bx-trending-up', 100)}
                    ${statCard('Net Balance', '₱0.00', 'Balance', 'bx-wallet', 100)}
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ${statCard('Journal Entries', '7', 'Recorded this period', 'bx-book-open', 100)}
                    ${statCard('Posted', '5', 'Confirmed entries', 'bx-check-double', 65)}
                    ${statCard('Draft', '1', 'Awaiting posting', 'bx-edit-alt', 15)}
                </div>
            </div>
            ...
            <div class="bg-card rounded-2xl border border-line shadow-brandSm overflow-hidden">
                <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4 border-b border-line">
                    <div class="flex items-center gap-2 text-sm font-semibold">
                        <button class="px-4 py-2 rounded-full bg-navy900 text-white">All Entries</button>
                        <button class="px-4 py-2 rounded-full bg-[#eef1f5] text-textMid hover:bg-[#e4e8ee]">Posted</button>
                        <button class="px-4 py-2 rounded-full bg-[#eef1f5] text-textMid hover:bg-[#e4e8ee]">Draft</button>
                        <button class="px-4 py-2 rounded-full bg-[#eef1f5] text-textMid hover:bg-[#e4e8ee]">Voided</button>
                    </div>
                    <div class="flex items-center gap-2 flex-wrap">
                        <div class="relative">
                            <i class='bx bx-search absolute left-3 top-1/2 -translate-y-1/2 text-orange'></i>
                            <input type="text" placeholder="Search Entries..." class="pl-9 pr-3 py-2 rounded-lg bg-[#eef1f5] border-none text-sm text-textDark placeholder:text-textSoft focus:outline-none focus:ring-2 focus:ring-navy900/20">
                        </div>
                        <select class="px-3 py-2 rounded-lg border border-line text-sm text-textMid bg-white"><option>All Accounts</option></select>
                        <select class="px-3 py-2 rounded-lg border border-line text-sm text-textMid bg-white"><option>All Months</option></select>
                        <button class="flex items-center gap-2 px-4 py-2 rounded-full bg-navy900 text-white text-sm font-semibold hover:bg-navyDark"><i class='bx bx-export text-orange'></i>Export CSV</button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm border-separate border-spacing-0">
                        <thead>
                            <tr class="text-left text-[11px] uppercase tracking-wide text-textSoft border-b border-line">
                                <th class="px-5 py-3 font-semibold"><input type="checkbox" class="rounded border-line"></th>
                                <th class="px-2 py-3 font-semibold">JE No.</th>
                                <th class="px-2 py-3 font-semibold">Date</th>
                                <th class="px-2 py-3 font-semibold">Accounts</th>
                ...
                <div class="flex items-center justify-between px-5 py-4 border-t border-line">
                    <div></div>
                    <div class="flex items-center gap-2">
                        <button class="w-8 h-8 rounded-lg bg-orange text-navy900 flex items-center justify-center"><i class='bx bx-chevron-left'></i></button>
                        <button class="w-8 h-8 rounded-lg bg-navy900 text-white text-sm font-semibold">1</button>
                        <button class="w-8 h-8 rounded-lg text-textMid text-sm font-semibold hover:bg-[#eef1f6]">2</button>
                        <button class="w-8 h-8 rounded-lg text-textMid text-sm font-semibold hover:bg-[#eef1f6]">3</button>
                        <span class="text-textSoft px-1">...</span>
                        <select class="border border-line rounded-lg text-sm px-2 py-1.5 text-textMid"><option>10</option></select>
                        <button class="w-8 h-8 rounded-lg bg-orange text-navy900 flex items-center justify-center"><i class='bx bx-chevron-right'></i></button>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    function renderComingSoon(title) {
        ...
        <div class="p-6">
            <div class="bg-card rounded-2xl border border-line shadow-brandSm p-10 text-center text-textMid">
                <i class='bx bx-wrench text-3xl text-textSoft mb-3'></i>
                <div class="font-semibold text-textDark">${title}</div>
                <div class="text-sm mt-1">This page hasn't been built yet.</div>
            </div>
        </div>`;
    }
    ...
    function renderPage(page) {
        ...
        if (page === 'ledger') {
            mainContent.innerHTML = renderGeneralLedger();
        } else {
            const titleEl = document.querySelector(`.nav-link[data-page="${page}"] .flex.flex-col span:first-child`);
            mainContent.innerHTML = renderComingSoon(titleEl ? titleEl.textContent : 'Page');
        }
    }
